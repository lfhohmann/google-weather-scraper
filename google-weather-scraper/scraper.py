...
def _get_wind(soup, output_units):
    ...
    for idx, _ in enumerate(data):
        ...
        if input_units == "metric":
            ...
            if output_units["speed"] == "mph":
                data[idx][0] = converters.kmph_to_mph(data[idx][0])

        else:
            data[idx][0] = float(data[idx][0].replace("mph", ""))

            if output_units["speed"] == "km/h":
                data[idx][0] = converters.mph_to_kmph(data[idx][0])
...
def get_forecast(header, url, region, output_units={"temp": "c", "speed": "km/h"}):
...
if __name__ == "__main__":
    data = get_forecast(HEADER, URL, "curitiba", {"temp": "c", "speed": "km/h"})
    pprint(data)
